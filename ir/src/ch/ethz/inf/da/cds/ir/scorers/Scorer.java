...
public class Scorer {
    ...
    private static double getQueryNorm(List<Term> terms, long docCount, IndexReader reader)
            throws IOException {
    ...
    public static Features[] getFeatures(String field, TrecQuery trecQuery, IndexReader reader)
            throws Exception {
        ...
        for (Term queryTerm : queryTerms) {
            ...
            for (LeafReaderContext ctx : reader.leaves()) {
                ...
                while ((docId = postingsEnum.nextDoc()) != PostingsEnum.NO_MORE_DOCS) {
                    ...
                    minTfIdf[globalDocId] = Math.min(tfidf, minTfIdf[globalDocId]);
                    maxTfIdf[globalDocId] = Math.max(tfidf, maxTfIdf[globalDocId]);
        ...
        for (int docId = 0; docId < reader.maxDoc(); docId++) {
            ...
            SummaryStats tfStats = new SummaryStats(sumTfs[docId],
                                                    minTf[docId],
                                                    maxTf[docId],
                                                    sumTfs[docId] / overlaps[docId]);
            SummaryStats normTfStats = new SummaryStats(sumTfs[docId] / fieldLength,
                                                        minTf[docId] / fieldLength,
                                                        maxTf[docId] / fieldLength,
                                                        sumTfs[docId] / (overlaps[docId] * fieldLength));
            SummaryStats tfIdfStats = new SummaryStats(sumTfIdfs[docId],
                                                       minTfIdf[docId],
                                                       maxTfIdf[docId],
                                                       sumTfIdfs[docId] / overlaps[docId]);
    ...
    public static void writeTopScores(Features[] scores, int queryId, int num, PrintWriter pw,
            int[] docIdMapping, Measure measure) throws Exception {
        PriorityQueue<Pair<Integer, Double>> sortedScores = getSortedScores(scores, measure);
        int rank = 1;
        for (int i = 0; i < num; i++) {
            Pair<Integer, Double> docScore = sortedScores.poll();
    ...
    public static void writeTopScores(double[] scores, int queryId, int num, PrintWriter pw,
            int[] docIdMapping) {
