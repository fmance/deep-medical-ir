...
public class XmlToPlaintextConverter {
	...
	private static void convertPmcDirectory(File pmcDir, File plaintextDir, List<String> validDocIds) throws Exception {
		...
		for (File subdir : subdirs) {
			executor.submit(
					new ConverterThread(subdir, plaintextDir.toPath().resolve(subdir.getName()).toFile(), validDocIds));
		}
		ThreadUtils.shutdownExecutor(executor);

		long end = System.currentTimeMillis();
		System.out.println(
				"Converted " + pmcDir.toPath().normalize() + ", took " + (end - begin) / (1e3 * 60) + " minutes\n\n");
	}
		...
		public void run() {
			...
			for (File articleFile : srcDir.listFiles()) {
				...
				if (filesWritten % 500 == 0) {
					...
					System.out.println("Directory " + srcDir.toPath().normalize() + ": written " + filesWritten
							+ " files, took " + (current - start) / 1e3 + " seconds.");
				}
			}
