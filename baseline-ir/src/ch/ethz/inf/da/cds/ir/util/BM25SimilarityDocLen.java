 ...
 * <p/>
 * BM25L and BM25+ improved versions of BM25. Introduced in Yuanhua Lv,
 * ChengXiang Zhai. "Lower-Bounding Term Frequency Normalization". In
...
public class BM25SimilarityDocLen extends Similarity {
    ...
    private class BM25DocScorer extends SimScorer {
        ...
        public float score(int doc, float freq) {
            ...
            if (model == BM25Model.CLASSIC) {
                return weightValue * freq / (freq + k1 * norm);
            } else if (model == BM25Model.L) {
                float normFreq = freq / norm;
                if (normFreq > 0) {
                    return weightValue * (normFreq + d) / (k1 + normFreq + d);
                } else {
                    return 0;
                }
            } else {
                return weightValue / (k1 + 1) * (freq * (k1 + 1) / (freq + k1 * norm) + d);
            }
        }
    ...
    private Explanation explainTFNorm(int doc, Explanation freq, BM25Stats stats, NumericDocValues norms) {
        ...
        if (norms == null) {
            ...
            return Explanation.match((freq.getValue() * (k1 + 1))
                    / (freq.getValue() + k1 * (1 - b + b * doclen / stats.avgdl)), "tfNorm, computed from:",
                    subs);
        }
    }
