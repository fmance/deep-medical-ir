...
\centerline{
  ...
  \caption{Components of the scoring function.}
  \label{diagram}
\end{figure}
...
\section{Fusion of Relevance and Intent Type Scores}
...
scoring function (Formula \ref{bm25-formula}) for query $Q$.
...
(of course, before normalizing the \textsf{TypeClassifier} scores, we first normalize its
two components from Formula \ref{clf-combine-formula} --- the
$\textsf{MLClassifier}$ and the \textsf{KeywordCounter} scores).
