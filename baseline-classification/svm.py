...
class_id = "treat"
class_dir = "res/" + class_id + "/"

data_train.data = (codecs.open(class_dir + "train/docs.txt", "r", "utf-8").readlines())
data_train.target = (map(lambda x: int(x), codecs.open(class_dir + "train/labels.txt", "r", "utf-8").readlines()))

data_test.data = codecs.open(class_dir + "test/docs.txt", "r", "utf-8").readlines()
data_test.target = map(lambda x: int(x), codecs.open(class_dir + "test/labels.txt", "r", "utf-8").readlines())

data_res.data = codecs.open(class_dir + "res/docs.txt", "r", "utf-8").readlines()
...
def benchmark(clf):
    ...
    f = open(class_dir + "predictions-on-ir-res.txt." + str(clf).split('(')[0], "w")
    for p in pred:
        f.write("%d\n" % p)
    f.close()
...
#        (BernoulliRBM(n_iter=1), "BernoulliRBM"),
...
#                                            dual=False, tol=1e-3)))
    ...
    # Train SGD model
    results.append(benchmark(SGDClassifier(alpha=.0001, n_iter=500,
                                           penalty=penalty)))
